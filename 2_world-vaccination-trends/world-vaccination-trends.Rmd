...
world_vaccination <- read_csv(here("data","world_vaccination.csv"))
 ...
 filter_vaccine <- world_vaccination %>% 
...
world_vacc_plot <- filter_vaccine %>%
  ggplot(aes(x = yr, color = vaccine, shape = factor(vaccine), y = pct_vaccinated)) +
  geom_point()
...
```{r}
polio <- filter(filter_vaccine, vaccine == "polio")
polio
```
...
```{r}
polio_regions <- polio %>%
  ggplot(aes(x = yr, color = who_region, shape = factor(who_region), y = pct_vaccinated)) +
  geom_point()
...
polio_regions_line <- polio %>%
  ggplot(aes(x = yr, color = who_region, y = pct_vaccinated)) +
  geom_line() +
...
```{r}
vaccine_regions_line <- filter_vaccine %>%
  ggplot(aes(x = yr, color = who_region, y = pct_vaccinated)) +
  geom_line() +
  labs(color = "World Region") +
  facet_wrap(~vaccine)
